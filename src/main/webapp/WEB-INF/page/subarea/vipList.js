layui.config({
	...
	var form = layui.form,table = layui.table;
		...
		table.render({
			id:'levelList',
		    elem: '#levelList'
		    ,url: ctx+'/subarea/getViplist/'+sbId //数据接口
		    ,cellMinWidth: 80
		    ,limit:10//每页默认数
		    ,limits:[10,20,30,40]
		    ,cols: [[ //表头
		  ...
		  table.on('tool(test)', function(obj){
		    ...
		    if(obj.event === 'edit'){
		    	if(data.status==0&&data.status2==0){
                    layer.confirm('真的关联行么?', function(index){
                        $.ajax({
                            url:ctx+'/subarea/guanlianVIP?leId='+data.leId+"&sbId="+sbId,
                            type : "get",
                            success : function(d){
                                if(d.code==0){
                    ...
                    layer.confirm('真的取消关联吗?', function(index){
                        $.ajax({
                            url:ctx+'/subarea/quxiaoVIP?leId='+data.leId+"&sbId="+sbId,
                            type : "get",
                            success : function(d){
                                if(d.code==0){
                                    table.reload('levelList', {})
