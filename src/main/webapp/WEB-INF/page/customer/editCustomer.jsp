...
<body class="childrenBody">
	...
	<input type="hidden" name="id" value="${customer.id }"/>
		<div class="layui-form-item">
			<label class="layui-form-label">电话类型</label>
			<div class="layui-input-block">
				<select name="type" class="" id="type" lay-verify="required">
					<option value="-1">请选择电话类型</option>
					<option value="1" <c:if test="${1==customer.type }">selected='selected'</c:if>>客服电话</option>
					<option value="2" <c:if test="${2==customer.type }">selected='selected'</c:if>>售后电话</option>
				</select>
			</div>
		</div>
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" class="layui-input userName"
					lay-verify="phone" placeholder="请输入手机号" name="tel" value="${customer.tel }">
			</div>
		</div>
