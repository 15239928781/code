layui.config({
    ...
    table.render({
        id:'carouselList',
        elem: '#carouselList'
        ,url: ctx+'/sys/getCategory2List' //数据接口
        ,cellMinWidth: 80
        ,limit:10//每页默认数
        ,limits:[10,20,30,40]
        ,cols: [[ //表头
            ...
            ,{field:'pName', title: '父列表名称'}
            ,{field:'cName', title: '列表名称'}
            ,{field:'cIsshow', title: '是否展示', templet: '#statusTpl', unresize: true},
            {
                field : 'cSorting',
                title : '排序',
                event: 'cSorting',
    ...
    table.on('tool(test)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('真的删除行么?', function(index){
                $.ajax({
                    url:ctx+'/sys/delCategory2ById?id='+data.cId,
                    type : "get",
                    success : function(d){
                        if(d.code==0){
            ...
            layer.open({
                ...
                title:"编辑列表信息",
                area: ['380px', '360px'],
                content:ctx+"/sys/editCategory2?id="+data.cId //这里content是一个普通的String
            })
        }
    ...
    $(".carouselAdd_btn").click(function(){
        layer.open({
            ...
            title:"添加列表信息",
            area: ['380px', '360px'],
            content:ctx+"/sys/addCategory2", //这里content是一个普通的String
        })
    })
...
//	      layer.alert(JSON.stringify(data));
        if(data.length>0){
            ...
            layer.confirm('真的要删除<strong>'+data.length+'</strong>条数据吗？', function(index){
                //调用删除接口
                $.ajax({
                    url:'delCategory2?carouselStr='+carouselStr,//接口地址
                    type : "get",
                    success : function(d){
                        if(d.code==0){
                            //重载表格
